...
void eyelash_detection(cv::Mat src, cv::Rect rectEye, cv::Mat &vis, cv::vector<cv::Point> &eyeLash)
{
    ...
    cv::namedWindow("Sobel2 eye", CV_WINDOW_NORMAL);          //// create window for display
    cv::imshow("Sobel2 eye", sob);
    ...
    cv::line(vis,ptLeft,ptRight,(125,0,255),2);

    cv::namedWindow("eyelash", CV_WINDOW_NORMAL);          //// create window for display
    cv::imshow("eyelash", vis);
